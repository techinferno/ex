    ...
    for word in text.split(' '):
        if word.upper() in operations.keys():
            try:
                ...
                list.delete(0,END)
                list.insert(END,'Somthing went wrong try again')
            finally:
                break
        elif word.upper() not in operations.keys():
            list.delete(0, END)
            list.insert(END, 'Something went wrong try again')
...
l1 = Label(win,text='Hey Im smart calculator',width = 20,padx=3)
l1.place(x=180,y=10)
l2 = Label(win,text='Done by Ayush',width = 20,padx=3)
l2.place(x=180,y=40)
